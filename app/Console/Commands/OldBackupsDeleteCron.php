...
class OldBackupsDeleteCron extends Command
{
    ...
    public function handle()
    {
        ...
        foreach($schedule as $item){
            ...
            if (count($daily) > 15) for($i = count($daily)-2; $i < count($daily); $i++){ $this->recursiveDelete($daily[$i]); }
            if (count($weekly) > 4) for($i = count($weekly)-2; $i < count($weekly); $i++){ $this->recursiveDelete($weekly[$i]); }
